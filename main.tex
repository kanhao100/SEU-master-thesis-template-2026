% 学硕
\documentclass[algorithmlist,figurelist,tablelist,nomlist,nocolorlinks,bookmarksnumbered]{template/seumasterthesis}
% 参数解析：algorithmlist算法列表, figurelist图片列表, tablelist表格列表, nomlist术语列表，engineer专硕, phd博士, nocolorlinks取消链接着色，bookmarksnumbered则PDF书签显示第一章、1.1、1.1.1（默认不显示章/节编号）。请根据需要选择你的参数。
% 专硕
% \documentclass[algorithmlist,figurelist,tablelist,nomlist,engineer,nocolorlinks,bookmarksnumbered]{template/seumasterthesis}
% 博士
% \documentclass[algorithmlist,figurelist,tablelist,nomlist,phd,nocolorlinks,bookmarksnumbered]{template/seumasterthesis}

%% ========== 参考文献：BST 样式与 BIB 数据库 ==========
%% 不同 bst 所需 bib 字段/格式可能不同，请对应切换 \seuBstStyle 与 \seuBibFiles。
% GB/T 7714-2015 国标
% \renewcommand{\seuBstStyle}{template/gbt7714-2015}
% \renewcommand{\seuBibFiles}{reference}
% 东南大学特定样式（区分硕士博士学位论文等，人名小写）
\renewcommand{\seuBstStyle}{template/seumasterthesis}
\renewcommand{\seuBibFiles}{reference}
% 可选：如果觉得参考文献条目之间偏松，可调条目间距
% 注意：这里必须带长度单位，如 0pt、-1pt、-2pt，不能写成 0.1 这种纯数字
% \seuBibItemSep{-2pt}
% % 如果想调条目内部换行的行距，再用下面这个参数
% \seuBibLineSpread{1.25}
...
\raggedbottom % 禁用强制底部对齐（取消注释则保持和Word一致）
...
%% ----------------------------------------------------------------------------
\title
    {东南大学 \LaTeX 论文模板使用手册:多行标题测试}        % 论文中文标题
...
\reviewer{}{}                % 两位论文评阅人姓名，盲审请填写盲审
\department                        % 学院名称
    {信息科学与工程学院}
    {School of Information Science and Engineering}
\seuthesisthanks                % 资助信息，没有请注释掉
...
% \seuListChapterVSpaceOff   % 单独控制目录章节间距，注释掉则需要间隙，不注释则与Word模板一致
% \seuTocChapterVSpaceOff   % 单独控制图表算法目录章节间距，注释掉则需要间隙，不注释则与Word模板一致
\seuTableOfContents      % 生成目录（书签锚点在 cls 内通过 \phantomsection 设置，与摘要、致谢等一致）
\input{chapters/nomenclature}  % 术语与符号表（nomlist 开启时由 \listofothers 输出）
...
\thesisbib                                   % 生成参考文献（bst/bib 由上文 \seuBstStyle、\seuBibFiles 控制）
